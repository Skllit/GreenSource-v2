    ...
    const fetchDeliveries = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:3800/api/delivery/agents/${agent?.data?._id}/pendingDeliveries`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Pending Deliveries</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">View and manage your pending delivery assignments</p>
        </div>
      ...
      {deliveries.length === 0 ? (
        <div className="bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm rounded-xl p-8 text-center border border-gray-200 dark:border-gray-700">
          ...
          <p className="text-gray-600 dark:text-gray-400">You don't have any pending deliveries at the moment.</p>
        </div>
      ) : (
