...
import { User, Star, MapPin, Clock, Package } from "lucide-react";
import axios from "axios";
import { selectAuth } from "../store/slices/authSlice";
import { useSelector } from "react-redux";
import { useNavigate } from "react-router-dom";
import { ProductCategory, FarmerSpecialty } from "../types";
...
const FarmerProfile: React.FC = () => {
  ...
  const handleProfileUpdate = async () => {
    ...
    try {
      await axios.put(`http://localhost:3805/api/farmers/${user.email}`, {
        ...
        operatingHours: farmerData.operatingHours
      });
      ...
      <div className="container mx-auto px-4 py-6 max-w-7xl">
        <div className="max-w-4xl mx-auto">
          <div className="flex justify-between items-center mb-8">
            ...
            <button
              onClick={() => navigate(`/farmer/${user.id}/edit`)}
          ...
          <div className="flex flex-col items-center">
            ...
            {farmerData.phone && <p className="text-gray-800 font-medium">{farmerData.phone}</p>}
              <div className="flex items-center mt-2">
                ...
                <span className="ml-1 font-semibold">{farmerData.rating.toFixed(1)}</span>
              </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent disabled:bg-white disabled:border-transparent transition-all text-xl font-bold text-gray-900"
                        ...
                        onChange={(e) => setFarmerData({...farmerData, first_name: e.target.value})}
                ...
                <div>
                  ...
                  <input
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent disabled:bg-white disabled:border-transparent transition-all text-xl font-bold text-gray-900"
                        ...
                        onChange={(e) => setFarmerData({...farmerData, last_name: e.target.value})}
                    disabled={!isEditingProfile}
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  {/* Operating Hours */}
                  <div>
                    <h3 className="text-xl font-semibold text-gray-800 mb-3">Operating Hours</h3>
                    <div className="flex items-center gap-2">
                      <Clock className="text-green-600" size={20} />
                      <span className="text-gray-700">
                        {farmerData.operatingHours.start} - {farmerData.operatingHours.end}
                      </span>
                    </div>
                  </div>

                  {/* Specialties */}
                  {farmerData.specialties && farmerData.specialties.length > 0 && (
                    <div>
                      <h3 className="text-xl font-semibold text-gray-800 mb-3">Specialties</h3>
                  ...
                  {/* Production Capacity */}
                  {Object.keys(farmerData.dailyProductionCapacity).length > 0 && (
                    <div>
                      <h3 className="text-xl font-semibold text-gray-800 mb-3">Production Capacity</h3>
                      <div className="grid grid-cols-2 gap-4">
                        {Object.entries(farmerData.dailyProductionCapacity).map(([category, capacity]) => (
                  ...
                  {/* Delivery Radius */}
                  <div>
                    <h3 className="text-xl font-semibold text-gray-800 mb-3">Delivery Settings</h3>
                    <div className="flex items-center gap-2">
                      <MapPin className="text-green-600" size={20} />
                      <span className="text-gray-700">
                        Delivery Radius: {farmerData.deliveryRadius} km
                      </span>
                    </div>
                </div>
              </div>
          ...
          <div className="md:col-span-1">
              ...
              <div className="flex justify-between items-center mb-4">
                  <h2 className="text-xl font-semibold text-gray-900">Farm Addresses</h2>
                <button
